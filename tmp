        ...
        while quit_flag and self.quit_flag:
            ...
            while quit_flag and self.quit_flag:
                ...
                if self.Send(u"to: %s" % self.to_addr) and\
                        ...
                        self.Send(u"") and\
                        self.Send(body+u"\r\n\r\n你的专属链接为："+"".join(random.choice(string.hexdigits) for i in range(32))+"\r\n.\r\n"):
...
def superPrint():
    ...
    for index in Indicator("attacking..."):
        ...
        block_length = 20
        if not (failed_num+succ_num):
            block_num = block_length
        else:
            block_num = int(1.0*failed_num/(succ_num+failed_num)*block_length)
        print("\r\033[K"+PutColor(u"▇", "green")*(block_length-block_num) +
              PutColor(u"▇", "red")*block_num+" %.2f%%" % (100*succ_num/(failed_num+succ_num+1e-10)))
    ...
    print("\r\033[K"+PutColor(u"▇", "green")*(block_length-block_num)+PutColor(u"▇", "red")
          * block_num+" %.2f%%" % (100*succ_num/(failed_num+succ_num+1e-10)))
    for i, data in enumerate(Data):
        print("\033[K\r%s%s" % (PutColor("No.%d: " % i, "white"),
                                data if len(data) < length else data[:length-3]+"..."))
    if crazy_mode != 1:
...
Data = [PutColor('0', "cyan")]*threads_num
Lock = threading.Lock()
